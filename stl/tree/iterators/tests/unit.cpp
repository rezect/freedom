...
class MapTest : public testing::Test {
 protected:
  void SetUp() override {
    mp.Insert({{1, 5}, {3, 10}, {5, 90}, {10, -10}, {90, 0}, {-10, 5}, {0, 4}});
    assert(mp.Size() == sz);
  }
...
TEST(EmptyMapTest, StringAsKey) {
  ...
  std::map<std::string, int> std_ages{
      {"Maxim", 21}, {"Danya", 22}, {"Veronika", 24}, {"Anna", 19}};
  auto values = ages.Values(true);
  auto it = values.begin();
  for (const auto& val : std_ages) {
    ASSERT_EQ(it->second, val.second) << fmt::format(
        "Values isn't equal on {} index", std::distance(values.begin(), it));
    ++it;
  }
}

TEST(EmptyMapTest, IteratorsEmptyTree) {
  Map<int, int> mp;
  ASSERT_EQ(mp.Begin(), mp.End());
}

TEST(EmptyMapTest, TreeFromExample) {
  Map<int, int> mp;
  mp.Insert({
      {10, 10},
      {8, 8},
...
TEST_F(MapTest, FindExistValue) {
  ASSERT_NE(mp.Find(3), mp.End());
}

TEST_F(MapTest, FindNotExistValue) {
  ASSERT_EQ(mp.Find(-11), mp.End());
}
